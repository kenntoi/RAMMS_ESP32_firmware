...
Mycila::PZEM pzem3; // 0x03
Mycila::PZEM pzem4; // 0x04
...
void setup() {
  ...
  // pzem4.begin(Serial2, 16, 17, 0x04, true);
  pzem1.begin(Serial1, 14, 27, 0x02, true);
  pzem2.begin(Serial1, 14, 27, 0x03, true);
  pzem3.begin(Serial2, 16, 17, 0x04, true);
...
void loop() {
  ...
  if (pzem4.isEnabled()) {
    JsonDocument doc;
    pzem4.toJson(doc.to<JsonObject>());
    Serial.printf("0x%02X ", pzem4.getDeviceAddress());
    serializeJson(doc, Serial);
    Serial.println();
  }
}